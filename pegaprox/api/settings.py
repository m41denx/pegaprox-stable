...
def check_cluster_updates(cluster_id):
    ...
    try:
        ...
        logging.error(f"[API] Failed to connect to cluster: {e}")
...
def start_rolling_update(cluster_id):
    ...
    try:
        ...
        logging.error(f"[API] Failed to get cluster nodes: {e}")
...
def update_node_repo(cluster_id, node, repo_id):
    ...
    try:
        ...
        logging.error(f"[API] Failed to update repository: {e}")
...
def refresh_node_repos(cluster_id, node):
    ...
    if getattr(mgr, 'cluster_type', 'proxmox') == 'xcpng':
        try:
            ...
            logging.error(f"[API] Failed to refresh packages: {e}")
    ...
    try:
        ...
        logging.error(f"[API] Failed to refresh repositories: {e}")
